defmodule NydusOperator.Controller.V1.Pod do
  ...
  defp parse(%{metadata: %{annotations: annotation} } = resource) do
    pod = annotation
      |> get_nydus_values()
      |> case do
        %{"enabled" => "true"} = config ->
...
defp get_nydus_values(annotation) do
  annotation
  |> Enum.filter(fn {k, _} -> Atom.to_string(k) =~ "nydus.mrchypark.github.io" end)
  |> Enum.map(fn {k, v} ->
      Atom.to_string(k)
      |> String.replace_prefix("nydus.mrchypark.github.io/", "")
      |> (&({&1, v})).()
    end)
    |> Enum.into(%{})
end
